...
export default class Categories extends React.Component{
    ...
    componentDidMount() { //sau khi component render sẽ chạy đến hàm này
        ...
        axios.get(API.categories.url) //lấy api.categories.url theo axios
    ...
    async deleteCategory(id){ //function del category theo id
        const rs = await axios.delete(API.delete_category.url+id); //tạo biến rs = hàm chờ xóa theo id của axios
                            ...
                            <tbody>
                            {
                                categories.map((e,k)=>{
                                    return <tr key={k}>
                                        <td>{e.id}</td>
                                        <td>{e.name}</td>
                                        <td><img src={e.image} width={50} height={50}/></td>
                                        <td>{e.description}</td>
                                        <td>
